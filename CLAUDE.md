...
Self-contained module under `src/game/modules/traders/`. Scenarios live in `src/game/modules/traders/board/scenarios/`. Same modular layout machinery as Seafarers / Fun Maps; `swamp`, `lake`, `wateringHole`, and `castle` are new non-producing terrains (treated like desert for token skipping). The current builds ship **Rivers of Catan**, **Fishing on Catan**, **Merchant Trains**, **Barbarian Attack**, plus the **Friendly Robber** and **Strongest Ports** variants — the Traders & Barbarians combo scenario and the Catan Event Cards / Catan for Two variants are deferred to follow-up commits.
...
- **Fishing on Catan** — `lake` hex (terrain type, fixed token via `fixedToken`) replaces the centre desert. `state.lakeHexId` records its id. `state.fishingGrounds: FishingGround[]` = 6 vertex-anchored tiles, each with its own number — declared via `ScenarioVertexRef`-style tuples and resolved to `VertexId`s in `generateTradersBoard`. Fish production runs after resource production in the dice handler via `maybeDistributeFish` (in `src/game/modules/traders/fishing/production.ts`). Tokens come from a face-down `state.fishTokenPool` (30 tokens: 11/10/8 + 1 old boot) which reshuffles `state.fishTokenDiscard` when empty. `player.fishTokens: Array<'one'|'two'|'three'>` is the per-player hand; `state.oldBootHolder` tracks the boot separately (boot doesn't count toward the 7-token cap). The 7-token cap is enforced as a hard stop on draws — the once-per-turn discard-and-replace nuance is deferred.
- **Robber off-board state** — Fishing starts with `state.robberActive = false`; the base dice / `maybeDistributeFish` / `BoardSVG` / `HexTile` all honor the flag. The robber re-activates on a 7 roll or Knight play. Fish spend can also remove the robber (cost 2) — `state.robberActive` flips back to false.
- **Spend fish (`spendFish` action)** — discriminated `effect.kind` covers 5 spends: `removeRobber` (cost 2), `steal` (3), `takeFromBank` (4), `buildRoad` (5), `buyDevCard` (7). All five are wired in the engine. UI ships only `removeRobber` + `takeFromBank` for now via `SpendFishDialog`; the other three are reachable only by direct dispatch / engine tests. Token sums ≥ cost; excess fish is forfeit to `fishTokenDiscard` per rulebook.
- **Old boot (`passOldBoot` action)** — held in `state.oldBootHolder`. Engine win-check uses `victoryPointsToWin + (currentId === oldBootHolder ? 1 : 0)` so the tax is per-player and only applies on your own win check. Boot pass legality uses public VPs (hidden VP dev cards don't count). `PassBootDialog` renders eligible recipients with their VP totals.
...
- **Merchant Trains** — `wateringHole` hex replaces the desert at the centre. `state.wateringHoleHexId` records its id; `state.wagons: TradeWagon[]` is the placed-wagon list (neutral pieces — `{ edge }`); `state.wagonSupply` counts what's left of the 22-token bag. Every successful `buildRoad`/`buildSettlement`/`buildCity`/`buildBridge` flips `state.builtThisTurn = true`. `handleEndTurnWithWagonVote` intercepts `endTurn`: when `builtThisTurn` AND a wateringHole is on the board, transitions to the `wagonVoting` phase instead of advancing the turn. The wrapper auto-fills AI bids with abstain on phase entry so the human-only voting dialog finishes the round.
- **Wagon voting** — `state.wagonVote: WagonVoteState` collects per-player bids (wool/wheat only). `handleSubmitWagonVote` deducts the bid from the player's hand and writes to `bids[playerId]`. When all `state.players.length` bids are in, `resolveWagonVote` refunds the cards to the bank (rulebook: bids go to the supply, not back), tallies votes per location and per player, then either auto-places via `placeWagonAndAdvance` (unique location winner) or transitions to `'placeWagon'` phase with `state.pendingWagonPlacement.placerId` set (unique player winner, or active-acquirer fallback). `placeWagonAndAdvance` calls the base `handleEndTurn` after placing — that's what actually rotates the turn.
- **Wagon placement** — `canPlaceWagon(state, edge)` in `merchantTrains/placement.ts` accepts edges adjacent to the watering hole OR sharing a vertex with exactly 1 existing wagon (a train endpoint). Merge points (degree ≥ 2) lock — wagons don't branch. The PlacementOverlay surfaces the `'placeWagon'` UI mode when the local seat is the chosen placer.
- **Merchant Trains scoring** — `calculateLongestRoad` adds +1 to path length for each traversed edge that ALSO has a wagon on it (rulebook: a wagon on a road edge counts as an extra road segment). `calculateMerchantTrainsVp` (`scoring/merchantTrains.ts`) adds +1 VP per settlement / city whose vertex has ≥ 2 incident wagon edges. Both feed into the master `calculateVictoryPoints`.
- **Barbarian Attack** — `castle` is a new non-producing terrain pinned to 3 sea-ring positions; `state.castles: CastleState[]` holds each castle's hex id, 4-hex `barbarianPath`, current `barbarianPosition` (0..3), `barbarianStrength` (default 4), and per-player `defenderVp` map. `state.barbarianKnightSupply` is the shared 18-knight bag; `player.defenderKnights: EdgeId[]` lists each player's hired knights. The `hireKnight` action (cost 1 wheat + 1 ore via `COSTS.hireKnight`) places a defender on an empty edge directly bounding a castle hex (6 slots per castle). The T&B `endTurn` wrapper (`src/game/modules/traders/turn.ts`) routes between Merchant Trains, Barbarian Attack, and base: in Barbarian Attack mode it calls `advanceBarbarians` then `resolveCombat` before delegating to base `handleEndTurn`. Barbarians advance deterministically one hex per turn; when one reaches its castle hex, combat resolves inline. Win (defenders ≥ strength): each defender earns +1 VP per knight contributed (stored on `castle.defenderVp`, surfaced through `calculateBarbarianDefenderVp` in `src/game/modules/traders/scoring/barbarianAttack.ts`); half the defending knights die (rounded up, picked via seeded `rngInt` — same pattern as Volcano). Loss (defenders < strength): all defending knights die; the defender with the fewest knights (or any player with a building on the castle's corners if no defenders) loses one settlement / city adjacent to the castle (cities downgrade, same as Volcano). Barbarian then resets to position 0.
- **AI** — `tryBuildBridge` (`src/ai/traders/bridges.ts`) fires between settlement and road in the priority tree when T&B is active. Threshold is 3.5 (vs. road's 4.5) because bridges pay +3 gold on top of network extension. `tryFishSpend` + `tryPassBoot` (`src/ai/traders/fish.ts`) fire just after bridges: drive off the robber when it sits on one of our hexes, take a needed resource when we have one in reach, and pass the boot to the highest-VP qualifier when we hold it. `vertexScore` adds +0.4 per adjacent swamp hex, +pips × 0.5 per adjacent lake hex, +pips × 0.9 for each fishing-ground anchor (vertex-exclusive so the bonus is concentrated), and +0.5 per adjacent castle hex (Barbarian Attack). For Merchant Trains, `trySubmitWagonVote` is invoked by the AI driver during `'wagonVoting'` and returns an abstain bid; `tryPlaceWagon` runs during `'placeWagon'` and prefers an edge that shares one of the AI's roads (extends Longest Route by +1). For Barbarian Attack, `tryHireKnight` (`src/ai/traders/knights.ts`) fires right after fish spends when a barbarian is within 2 hexes of a castle whose existing defense is below `barbarianStrength` — it grabs any open slot at the most-threatened castle.
...
- [~] Phase 10 — Traders & Barbarians expansion (in progress: Rivers of Catan + Fishing on Catan + Merchant Trains + Barbarian Attack scenarios shipping, plus Friendly Robber and Strongest Ports variants. The Traders & Barbarians combo scenario and the Catan Event Cards / Catan for Two variants are deferred to follow-up commits)
