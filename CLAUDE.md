...
Self-contained module under `src/game/modules/traders/`. Scenarios live in `src/game/modules/traders/board/scenarios/`. Same modular layout machinery as Seafarers / Fun Maps; `swamp` and `lake` are new non-producing terrains (treated like desert for token skipping). The current builds ship **Rivers of Catan**, **Fishing on Catan**, plus the **Friendly Robber** and **Strongest Ports** variants — Merchant Trains, Barbarian Attack, the Traders & Barbarians combo scenario, and the Catan Event Cards / Catan for Two variants are deferred to follow-up commits.
...
- [~] Phase 10 — Traders & Barbarians expansion (in progress: Rivers of Catan + Fishing on Catan scenarios shipping, plus Friendly Robber and Strongest Ports variants. Merchant Trains, Barbarian Attack, the Traders & Barbarians combo scenario, and the Catan Event Cards / Catan for Two variants are deferred to follow-up commits)
