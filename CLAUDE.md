...
Self-contained module under `src/game/modules/traders/`. Scenarios live in `src/game/modules/traders/board/scenarios/`. Same modular layout machinery as Seafarers / Fun Maps; `swamp`, `lake`, and `wateringHole` are new non-producing terrains (treated like desert for token skipping). The current builds ship **Rivers of Catan**, **Fishing on Catan**, **Merchant Trains**, plus the **Friendly Robber** and **Strongest Ports** variants — Barbarian Attack, the Traders & Barbarians combo scenario, and the Catan Event Cards / Catan for Two variants are deferred to follow-up commits.
...
- **AI** — `tryBuildBridge` (`src/ai/traders/bridges.ts`) fires between settlement and road in the priority tree when T&B is active. Threshold is 3.5 (vs. road's 4.5) because bridges pay +3 gold on top of network extension. `tryFishSpend` + `tryPassBoot` (`src/ai/traders/fish.ts`) fire just after bridges: drive off the robber when it sits on one of our hexes, take a needed resource when we have one in reach, and pass the boot to the highest-VP qualifier when we hold it. `vertexScore` adds +0.4 per adjacent swamp hex, +pips × 0.5 per adjacent lake hex, and +pips × 0.9 for each fishing-ground anchor (vertex-exclusive so the bonus is concentrated). For Merchant Trains, `trySubmitWagonVote` is invoked by the AI driver during `'wagonVoting'` and returns an abstain bid; `tryPlaceWagon` runs during `'placeWagon'` and prefers an edge that shares one of the AI's roads (extends Longest Route by +1).
...
- [~] Phase 10 — Traders & Barbarians expansion (in progress: Rivers of Catan + Fishing on Catan + Merchant Trains scenarios shipping, plus Friendly Robber and Strongest Ports variants. Barbarian Attack, the Traders & Barbarians combo scenario, and the Catan Event Cards / Catan for Two variants are deferred to follow-up commits)
